# Script che serve per generare le descrizioni per il dataset FashionIQ test
"""
1. Carica i file JSON originali (cap.toptee.test.json, cap.dress.test.json, cap.shirt.test.json)
...
5. Aggiunge il campo multi_caption_dam con le nuove descrizioni
...
# Configurazione
INPUT_FILES = [
    Path('./data/datasets/FashionIQ/captions/cap.toptee.test.json'),
    Path('./data/datasets/FashionIQ/captions/cap.dress.test.json'),
    Path('./data/datasets/FashionIQ/captions/cap.shirt.test.json')
]

OUTPUT_FILES = [
    Path('./data/files/toptee.test_dam.json'),
    Path('./data/files/dress.test_dam.json'),
    Path('./data/files/shirt.test_dam.json')
]

CHECKPOINT_FILES = [
    Path('./data/files/checkpoint_fiq_test_toptee_dam.json'),
    Path('./data/files/checkpoint_fiq_test_dress_dam.json'),
    Path('./data/files/checkpoint_fiq_test_shirt_dam.json')
]
...
def process_batch(batch: List[dict]) -> List[dict]:
    """Processa un batch di elementi FashionIQ test."""
    ...
    try:
        ...
        for idx, (original_idx, image_id) in enumerate(zip(valid_indices, valid_ids)):
            ...
            if descriptions and len(descriptions) >= NUM_CAPTIONS:
                # Crea una copia dell'item originale aggiungendo le descrizioni
...
def process_fashioniq_test_file(input_file: Path, output_file: Path, checkpoint_file: Path):
    """Elabora un singolo file FashionIQ test."""
    print(f"\n{'='*60}")
    print(f"🚀 PROCESSING TEST FILE: {input_file.name}")
    print(f"{'='*60}")
...
def main():
    # Processa tutti i file FashionIQ test
    all_results = []
    
    for input_file, output_file, checkpoint_file in zip(INPUT_FILES, OUTPUT_FILES, CHECKPOINT_FILES):
        results = process_fashioniq_test_file(input_file, output_file, checkpoint_file)
    ...
    print(f"\n🎉 TUTTI I FILE FASHIONIQ TEST SONO STATI PROCESSATI!")
