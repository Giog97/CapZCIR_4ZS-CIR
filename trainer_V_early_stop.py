...
class Trainer():
    ...
    def train(self):
        ...
        nan_counter = 0  # Contatore per NaN consecutivi --> aggiunto
        max_nan_tolerance = 5  # Numero massimo di NaN consecutivi tollerati --> aggiunto

        self.model.to(self.device)
        for epoch in range(self.num_epochs):
            # Esegui l'epoca di training e controlla se c'è NaN loss --> aggiunto
            epoch_has_nan  = self.train_epoch(epoch)
            #self.train_epoch(epoch)

            if epoch_has_nan: # EARLY STOPPING --> aggiunto
                nan_counter += 1
                print(f"NaN detected in epoch {epoch}. Counter: {nan_counter}/{max_nan_tolerance}")
                if nan_counter >= max_nan_tolerance:
                    print(f"Training stopped at epoch {epoch} due to consecutive NaN losses")
                    break
            else:
                nan_counter = 0  # Reset del contatore se non c'è NaN
            ...
            if epoch % self.validation_frequency == 0:
                ...
                if self.dataset == 'cirr':
                    ...
                    score= mean(results)
                # Salva il miglior modello basato sulla media aritmetica
                if score > best_score:
                    best_score = score
    ...
    def train_epoch(self, epoch):
        self.model.train()
        train_running_results = {'images_in_epoch': 0, 'accumulated_train_loss': 0} # NOTA: 'images_in_epoch'
        train_bar = tqdm(self.train_dataloader, ncols=150)
        iters = len(train_bar)
        epoch_has_nan = False # --> aggiunto
        for idx, batch in enumerate(train_bar):
            ...
            if not self.use_amp:
                ...
                # Controllo per NaN loss [EARLY STOPPING] --> aggiunto
                # Controllo per NaN loss
                if torch.isnan(loss):
                    print(f"NaN loss detected at batch {idx}, but continuing...")
                    epoch_has_nan = True
                    continue  # Salta questo batch ma continua l'epoca
        ...
        # wandb to log
        # Log della loss dell'epoca
        if train_running_results['images_in_epoch'] > 0:  # CAMBIA QUI
            train_epoch_loss = float(train_running_results['accumulated_train_loss'] / train_running_results['images_in_epoch'])  # CAMBIA QUI
            wandb.log({'train_epoch_loss': train_epoch_loss})
        else:
            train_epoch_loss = float('nan')
            print("Entire epoch had NaN losses!")
        
        return epoch_has_nan
