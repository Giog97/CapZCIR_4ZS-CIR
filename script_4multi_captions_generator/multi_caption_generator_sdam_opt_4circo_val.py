# Script che serve per generare le descrizioni per il dataset CIRCO validation
"""
1. Carica il file JSON originale val_circo_pavan.json
...
7. Salva il risultato in 'val_circo_dam.json'
"""
import json
...
# Configurazione
INPUT_JSON = Path('./data/files/val_circo_pavan.json')
IMAGE_DIR = Path('./data/datasets/CIRCO/COCO2017_unlabeled/unlabeled2017')  # Directory delle immagini CIRCO
OUTPUT_JSON = Path('./data/files/val_circo_dam.json')
CHECKPOINT_JSON = Path('./data/files/checkpoint_val_circo_dam.json')
NUM_CAPTIONS = 15
BATCH_SIZE = 4
...
def load_circo_image(ref_image_id: str) -> Image.Image:
    """
    Carica un'immagine CIRCO dal filesystem.
    Le immagini CIRCO hanno path: ./data/datasets/CIRCO/COCO2017_unlabeled/unlabeled2017/{ref_image_id}.jpg
    """
    try:
        image_path = IMAGE_DIR / f"{ref_image_id}.jpg"
...
def load_images_batch(ref_image_ids: List[str]) -> List[Image.Image]:
...
def main():
    ...
    print(f"\n🚀 AVVIO ELABORAZIONE CIRCO VALIDATION")
    print(f"{'='*60}")
