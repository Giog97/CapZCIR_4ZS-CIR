...
from model.captioner_sdam_singlepoint import CaptionerSDAM_singlepoint
import time
import gc
...
OUTPUT_JSON = Path('./data/files/laion_combined_dam_singlepoint.json')
CHECKPOINT_JSON = Path('./data/files/checkpoint_dam_singlepoint.json')
NUM_CAPTIONS = 3
BATCH_SIZE = 4  # Reduced from 8 to handle memory better
CHECKPOINT_INTERVAL = 50
MAX_WORKERS = 2  # Reduced from 4 to lower memory pressure

# Inizializza il captioner una sola volta
print("Inizializzazione del captioner...")
captioner = CaptionerSDAM_singlepoint()
...
def process_batch(batch: List[dict]) -> List[dict]:
    ...
    try:
        ...
        for image in images:
            try:
                # Clean memory before each image
                torch.cuda.empty_cache()
                gc.collect()
                
                # Process single image
                descriptions = captioner.generate_descriptions(image)
        ...
        for item, descriptions in zip(valid_items, batch_descriptions):
            if descriptions and len(descriptions) == NUM_CAPTIONS:
                batch_results.append({
                    ...
                    "multi_caption_dam": descriptions
                })
            else:
