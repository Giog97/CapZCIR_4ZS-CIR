# Script che serve per generare le descrizioni per il dataset CIRR e poter così fare validation con CIRR
# NB implementato per cap.rc2.val.json ma può essere facilmente modificato per cap.rc2.test1.json
"""
1. Carica il file JSON originale val_cirr_opt_laion_combined_multi_fixed.json (partenza da file di PAVAN)
--> se si vuole partite dal file originale di CIRR bisogna modificare questo script (file originale cap.rc2.val.json)
...
7. Salva il risultato in 'cap.rc2.val_dam.json'
"""
import json
...
# Configurazione
INPUT_JSON = Path('./data/files/val_cirr_opt_laion_combined_multi_fixed.json')
IMAGE_DIR = Path('./data/datasets/CIRR')  # Directory delle immagini CIRR
OUTPUT_JSON = Path('./data/files/cap.rc2.val_dam.json')
CHECKPOINT_JSON = Path('./data/files/checkpoint_cirr_val_dam.json')
NUM_CAPTIONS = 15
BATCH_SIZE = 4
