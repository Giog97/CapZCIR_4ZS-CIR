...
def generate_cirr_test_submissions(file_name, model, preprocess, device):
    ...
    capzcir_time = time.perf_counter()
    ...
    elapsed_time = end_time - capzcir_time
    print(len(pairid_to_predictions))
    print(len(pairid_to_group_predictions))
    print(elapsed_time)
    print(elapsed_time/len(pairid_to_predictions))
    submission = {
        'version': 'rc2',
        'metric': 'recall'
    }
...
def generate_cirr_test_predictions(relative_test_dataset: CIRRDataset, model, device) -> Tuple[torch.tensor, List[str], List[List[str]], List[str]]:
    ...
    relative_test_loader = DataLoader(dataset=relative_test_dataset, batch_size=1,
                                      num_workers=10, pin_memory=True, collate_fn=collate_fn)
    ...
    for batch_pairs_id, batch_reference_names, batch_reference_img_texts, captions, batch_group_members in tqdm(
            relative_test_loader):  # Load data
        batch_group_members = np.array(batch_group_members).T.tolist()
        batch_reference_img_texts1=np.array(batch_reference_img_texts).T.tolist()

        # Compute the predicted features
        with torch.no_grad():
            # reference_images = reference_images.to(device)
            batch_predicted_features = model.combine_features(batch_reference_img_texts1, captions)
    ...
    print('\nEstimated time:\nSAM {} s\nRQN {} s\nTransformer {} s\nText_encoder {} s\n'.format(round(model.sam / 4148, 5), round(model.rqn / 4148, 5), round(model.transformer /4148, 5), round(model.text_encoder_Time / 4148, 5)))
