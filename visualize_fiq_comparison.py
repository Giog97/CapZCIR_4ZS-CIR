# visualize_fiq_comparison.py
# Per lanciarlo:
# python visualize_fiq_comparison.py --num_queries 1000 --top_cases 10 --dress_types dress shirt toptee --output_dir ./my_comparisons_fiq_all
import json
import torch
import numpy as np
...
class FashionIQComparison:
    def __init__(self, config_1t, config_2tv, dress_types=['dress'], device='cuda'):
        self.device = device
        ...
        # Carica dataset
        print(f"Caricamento dataset FashionIQ per {dress_types}...")
        self.classic_dataset = FashionIQDataset(split='val', dress_types=dress_types, mode='classic', preprocess=self.preprocess)
        self.relative_dataset = FashionIQDataset(split='val', dress_types=dress_types, mode='relative', preprocess=self.preprocess)
        ...
        for dress_type in dress_types:
            with open(f'./data/files/{dress_type}.val_dam.json', 'r') as f:
    ...
    def get_image_path(self, image_name):
        ...
        # Rimuovi l'estensione .jpg se presente
        if image_name.endswith('.jpg'):
            image_name = image_name[:-4]
        return f"./data/datasets/FashionIQ/images/{image_name}.jpg"
    
    def retrieve_images(self, model, reference_name, reference_texts, rel_captions, k=10):
        """Esegue retrieval per una query specifica"""
        with torch.no_grad():
            # Prepara l'input - FashionIQ usa una lista di caption invece di una singola
            reference_texts = [reference_texts]  # Lista di liste di caption
            rel_caption = [' '.join(rel_captions)]  # Combina le caption relative in una stringa
    ...
    def evaluate_query(self, query_idx, k=10):
        ...
        rel_captions = triplet['captions']  # FashionIQ usa una lista di caption
        reference_texts = [str(x) for x in triplet["multi_caption_opt"]]
        
        print(f"Processing query {query_idx}: {' '.join(rel_captions)}")
        
        # Retrieval con BLIP 1t
        retrieved_1t, scores_1t = self.retrieve_images(
            self.model_1t, reference_name, reference_texts, rel_captions, k
        )
        
        # Retrieval con BLIP 2tv
        retrieved_2tv, scores_2tv = self.retrieve_images(
            self.model_2tv, reference_name, reference_texts, rel_captions, k
        )
    ...
    def find_improvement_cases(self, num_queries=100, top_k=10):
        ...
        for i in range(min(num_queries, len(self.triplets))):
            try:
                ...
                target = results['query_info']['target']
                ...
                if improvement > 0:  # Solo casi con miglioramento
                    improvements.append({
                        ...
                        'captions': results['query_info']['captions'],
                        'results': results
                    })
    ...
    def visualize_comparison(self, results, save_path=None):
        ...
        # FashionIQ ha multiple caption, le combiniamo
        combined_caption = ' '.join(query["captions"])
        wrapped_caption = textwrap.fill(combined_caption, width=50)
        ...
        # Reference
        ref_img = Image.open(self.get_image_path(query['candidate']))
        ax_ref = fig.add_subplot(gs[0, 6:8]) #7:9 #8:10
        ...
        ax_tgt = fig.add_subplot(gs[0, 8:10]) #9:11 #10:12
        ax_tgt.imshow(tgt_img)
        ax_tgt.set_title("Target", fontsize=16, weight='bold', color='green')
        ax_tgt.axis("off")

        # ---- Function to plot retrieved with fixed height ----
        def plot_row(imgs, scores, row, title):
            ...
            #target_height = 150  # Altezza fissa in pixel per tutte le immagini
...
def main():
    parser = argparse.ArgumentParser(description='Confronto BLIP 1t vs BLIP 2tv su FashionIQ')
    parser.add_argument('--dress_types', nargs='+', default=['dress'], 
                       choices=['dress', 'shirt', 'toptee'], 
                       help='Categorie FashionIQ da analizzare')
    ...
    parser.add_argument('--output_dir', type=str, default='./fiq_comparison_results', help='Directory di output')
    
    args = parser.parse_args()
    
    # Crea directory output specifica per la categoria
    category_str = '_'.join(args.dress_types)
    output_dir = os.path.join(args.output_dir, category_str)
    ...
    print(f"Inizializzazione FashionIQ Comparison per {args.dress_types}...")
    comparator = FashionIQComparison(config_1t, config_2tv, dress_types=args.dress_types)
    ...
    for i, case in enumerate(improvements[:args.top_cases]):
        ...
        print(f"   Captions: {' '.join(case['captions'])}")
        ...
        save_path = os.path.join(output_dir, f'comparison_case_{i+1}_idx{case["index"]}.png')
        comparator.visualize_comparison(case['results'], save_path=save_path)
    
    # Salva riepilogo
    summary_path = os.path.join(output_dir, 'improvement_summary.txt')
    with open(summary_path, 'w') as f:
        f.write(f"RIEPILOGO MIGLIORAMENTI BLIP 2tv vs BLIP 1t - {args.dress_types}\n")
        f.write("=" * 60 + "\n\n")
        
        for i, case in enumerate(improvements[:args.top_cases]):
            f.write(f"Caso {i+1} (Index: {case['index']}):\n")
            f.write(f"Captions: {' '.join(case['captions'])}\n")
